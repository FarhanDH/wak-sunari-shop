...
export class UsersService {
  constructor(
    ...
    private userModel: Model<schema.User>,
  ) {}
  ...
  async register(createUserDto: CreateUserDto) {
    ...
    const createdUsers = new this.userModel({
      ...createUserDto,
  ...
  //     if (usernameCheck) throw new ConflictException('Username already exists');
  //   }
  //   if (email) {
  //     const emailCheck = await this.findByEmail(email);
  //     if (emailCheck) throw new ConflictException('Email already exists');
  //   }
  //   return {
  //     message: 'Available',
  //   };
  ...
  //   if (!userWithPosts) throw new NotFoundException('User Profile Not Found');

  //   // get post data with number of comments for each post and isUserLiked for each post
  //   userWithPosts.posts = userWithPosts.posts.map(({ likes, ...post }, i) => {
  //     return {
  //       ...post,
  //       commentsNumber:
  ...
  //               .map((comment) => {
  //                 if (comment.replies) {
  //                   return this.commentsService.addRepliesCount(comment);
  //                 }
  //               })
  //               .reduce(
